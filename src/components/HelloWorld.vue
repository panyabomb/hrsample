...
<v-flex xs6>
 <v-card-text>
     ...
     required
       ...
       :customStrings="{
         upload: '<h1>Bummer!</h1>',
         drag: 'คลิกเพื่อใส่รูปบัตรประชาชน'
       }">
     </picture-input>
...
<v-alert
   ...
   transition="scale-transition"
 >
   !!อย่าเปลี่ยนหน้าเว็บ กรุณารอสักครู่ ระบบกำลังอัปโหลดข้อมูล
 </v-alert>
<v-btn
     @click="submit"
     :disabled="!valid"
...
<script>
  import {db, str} from '@/components/firebase'
    ...
    methods: {
      ...
      async submit () {
        if (this.$refs.form.validate()) {
          this.datas.information.id = Date.now() + Math.random()
          ...
          try {
            ...
            db.ref().child('data').push(this.datas)
            console.log('complete!')
            alert('อัปโหลดข้อมูลเสร็จเรียบร้อย')
            location.reload()
          } catch (error) {
            console.log(error)
          }
